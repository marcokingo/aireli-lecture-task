...
export const createUser = async(newUser: PlatformUser): Promise<any> => {
  const db = await openDb();
  const hashedPassword = await bcrypt.hash(newUser.password, 10); // Hash the password before storing
